...
public class T1Mapper extends TableMapper<Text, LongWritable> {
	public void map(ImmutableBytesWritable key, Result value, Context context)
		...
		try {
			timeStart = dateFormat.parse(context.getConfiguration().getStrings(
					"daterange")[0]);
			timeEnd = dateFormat.parse(context.getConfiguration().getStrings(
					"daterange")[1]);
		...
		if (ts.after(timeStart) && ts.before(timeEnd)) {
			...
			long revid = Bytes.toLong(Arrays.copyOfRange(key.get(), 8,
					key.getLength()));
			// Output result
